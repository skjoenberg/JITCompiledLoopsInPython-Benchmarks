...
t_arr  = np.array([4/9., 1/9.,1/9.,1/9.,1/9., 1/36.,1/36.,1/36.,1/36.])
cx      = [  0,   1,  0, -1,  0,    1,  -1,  -1,   1]
cx_arr  = np.array([  0,   1,  0, -1,  0,    1,  -1,  -1,   1])
cy      = [  0,   0,  1,  0, -1,    1,   1,  -1,  -1]
cy_arr  = np.array([  0,   0,  1,  0, -1,    1,   1,  -1,  -1])
...
def solve(state, iterations, B, viz=None):
    ...
    for cycle in range(iterations):
      ...
      def loop_body(cx, ux, cy, uy, fEq, fOut, fIn, omega, rho, t):
          ...
          fEq[i] = rho * t[i:i+1] * (1 + cu[i:i+1] + 0.5 * cu[i:i+1] ** 2 - 1.5 * (ux ** 2 + uy ** 2))
          fOut[i] = fIn[i:i+1] - omega * (fIn[i:i+1] - fEq[i:i+1])

      B.do_while(loop_body, 9, cx_arr, ux, cy_arr, uy, fEq, fOut, fIn, omega, rho, t_arr)
      ...
      if bbRegion is not None:
          B.do_while(loop_body3, 9, np.array(cx), ux, np.array(cy), uy, fEq, fOut, fIn, np.array(t), ly, lx)
      else:
          B.do_while(loop_body2, 9, cx_arr, ux, cy_arr, uy, fEq, fOut, fIn, t_arr, ly, lx)
